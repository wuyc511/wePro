package com.feiying.erp.config;
...
public class TenantConfig {
    ...
    public PaginationInterceptor paginationInterceptor(HttpServletRequest request) {
                ...
                if(mybatisPlusStatus !=null && mybatisPlusStatus.toString().equals("open")) {
                    ...
                    if(("admin").equals(loginName)) {
                        ...
                        if ("databasechangelog"
                                ...
                                || "feiying_materialproperty".equals(tableName)
                                || "tbl_sequence".equals(tableName)
                                || "dual".equals(tableName)
                                || "feiying_userbusiness".equals(tableName)
                                || "feiying_app".equals(tableName)
                                || "feiying_functions".equals(tableName)
                                || "feiying_tenant".equals(tableName)) {
                            return true;
                        } else {
                            return false;
                        }
        ...
        paginationInterceptor.setSqlParserFilter(new ISqlParserFilter() {
            ...
            public boolean doFilter(MetaObject metaObject) {
                ...
                // 过滤自定义查询此时无租户信息约束出现
                //if ("DepotHeadMapperEx.getBuildOnlyNumber".equals(ms.getId())) {
                //    return true;
                //}
                return false;
            }
    ...
    /**
     * 相当于顶部的：
     * {@code @MapperScan("com.feiying.erp.datasource.mappers*")}
     * 这里可以扩展，比如使用配置文件来配置扫描Mapper的路径
     */
    @Bean
    public MapperScannerConfigurer mapperScannerConfigurer() {
        MapperScannerConfigurer scannerConfigurer = new MapperScannerConfigurer();
        scannerConfigurer.setBasePackage("com.feiying.erp.datasource.mappers*");
